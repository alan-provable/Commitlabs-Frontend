...
const POST = postHandler as (req: NextRequest, context: { params: Record<string, string> }) => Promise<Response>;
  ...
  it('maps normalized contract errors into the standard error envelope', async () => {
    mockedEarlyExitCommitmentOnChain.mockRejectedValue(
      new BackendError({
        ...
        status: 504,
        details: { method: 'early_exit_commitment', commitmentId: COMMITMENT_ID, retryable: true },
      })
    );
